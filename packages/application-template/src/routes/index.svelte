	...
	<section class="min-h-screen flex justify-center items-center">
		<ul role="list" class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
			{#each posts as post, i}
				<li class="col-span-1 bg-white rounded-lg shadow divide-y divide-gray-200" data-speed="{calcDataSpeed(i)}">
					<div class="w-full flex items-center justify-between p-6 space-x-6">
						<div class="flex-1 truncate">
							...
							<p class="mt-1 text-gray-500 text-sm truncate">{post.excerpt}</p>
						</div>
						<img class="w-10 h-10 bg-gray-300 rounded-full flex-shrink-0" src="{post.author.picture.url}" alt=""/>
					</div>
